...
public class MySQL5PlayerBindPointDAO extends PlayerBindPointDAO {
    ...
    public void loadBindPoint(Player player) {
        ...
        try {
            ...
            log.error("Could not restore BindPointPosition data for playerObjId: " + player.getObjectId() + " from GDB: " + e.getMessage(), e);
        } finally {
    ...
    public boolean insertBindPoint(Player player) {
        ...
        try {
            ...
            stmt.setByte(6, (byte) bpp.getHeading());
            stmt.execute();
            stmt.close();
        } catch (Exception e) {
            log.error("Could not store BindPointPosition data for player " + player.getObjectId() + " from GDB: " + e.getMessage(), e);
            return false;
        } finally {
            DatabaseFactory.close(con);
        }
    ...
    public boolean updateBindPoint(Player player) {
        ...
        try {
            ...
            stmt.setByte(5, (byte) bpp.getHeading());
            stmt.setFloat(6, player.getObjectId());
            stmt.execute();
            stmt.close();
        } catch (Exception e) {
            log.error("Could not update BindPointPosition data for player " + player.getObjectId() + " from GDB: " + e.getMessage(), e);
            return false;
        } finally {
            DatabaseFactory.close(con);
        }
