...
public class MySQL5AbyssRankDAO extends AbyssRankDAO {
    ...
    public static final String SELECT_QUERY = "SELECT daily_ap, weekly_ap, ap, rank, top_ranking, daily_kill, weekly_kill, all_kill, max_rank, last_kill, last_ap, " +
        "last_update FROM abyss_rank WHERE player_id = ?";
    public static final String INSERT_QUERY = "INSERT INTO abyss_rank (player_id, daily_ap, weekly_ap, ap, rank, top_ranking, daily_kill, weekly_kill, all_kill, max_rank, last_kill, last_ap, " +
        "last_update) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    public static final String UPDATE_QUERY = "UPDATE abyss_rank SET  daily_ap = ?, weekly_ap = ?, ap = ?, rank = ?, top_ranking = ?, daily_kill = ?, weekly_kill = ?, all_kill = ?, max_rank = ?, " +
        "last_kill = ?, last_ap = ?, last_update = ? WHERE player_id = ?";
    public static final String SELECT_PLAYERS_RANKING = "SELECT abyss_rank.rank, abyss_rank.ap, abyss_rank.old_rank_pos, abyss_rank.rank_pos, players.name, legions.name, players.id, " +
        "players.title_id, players.player_class, players.exp FROM abyss_rank INNER JOIN players ON abyss_rank.player_id = players.id LEFT JOIN legion_members ON legion_members.player_id = players" +
        ".id LEFT JOIN legions ON legions.id = legion_members.legion_id WHERE players.race = ? AND abyss_rank.ap > 0 ORDER BY abyss_rank.ap DESC LIMIT 0, 300";
    public static final String SELECT_LEGIONS_RANKING = "SELECT legions.id, legions.name, legions.contribution_points, legions.level as lvl, legions.old_rank_pos, legions.rank_pos FROM legions," +
        "legion_members,players WHERE players.race = ? AND legion_members.rank = 'BRIGADE_GENERAL' AND legion_members.player_id = players.id AND legion_members.legion_id = legions.id AND legions" +
        ".contribution_points > 0 GROUP BY id ORDER BY legions.contribution_points DESC LIMIT 0,50";
    ...
    public static final String UPDATE_PLAYER_RANK_LIST = "UPDATE abyss_rank SET abyss_rank.old_rank_pos = abyss_rank.rank_pos, abyss_rank.rank_pos = @a:=@a+1 where player_id in (SELECT id FROM " +
        "players where race = ?) order by ap desc"
        + (GSConfig.ABYSSRANKING_SMALL_CACHE ? " limit 500" : "");  // only 300 positions are relevant later, so we update them + some extra positions that
    // can get into the toprankings
    public static final String UPDATE_LEGION_RANK_LIST = "UPDATE legions SET legions.old_rank_pos = legions.rank_pos, legions.rank_pos = @a:=@a+1 where id in (SELECT legion_id FROM legion_members, " +
        "players where rank = 'BRIGADE_GENERAL' AND players.id = legion_members.player_id and players.race = ?) order by legions.contribution_points DESC"
        + (GSConfig.ABYSSRANKING_SMALL_CACHE ? " limit 75" : ""); // only 50 positions are relevant later, so we update them + some extra positions that can
    ...
    public ArrayList<AbyssRankingResult> getAbyssRankingPlayers(Race race) {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                AbyssRankingResult rsl = new AbyssRankingResult(name, playerAbyssRank, playerId, ap, playerTitle, playerClass, playerLevel, playerLegion,
                    oldRankPos, rankPos);
                results.add(rsl);
            }
            resultSet.close();
    ...
    public ArrayList<AbyssRankingResult> getAbyssRankingLegions(Race race) {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                long cp = resultSet.getLong("legions.contribution_points");
            ...
            log.error("getAbyssRankingLegions", e);
        } finally {
            DatabaseFactory.close(con);
        }
        return results;
    ...
    /*
     * (non-Javadoc)
     * @see com.ne.gs.database.dao.AbyssRankDAO#updateRankList()
     */
    @Override
