...
public class MySQL5MailDAO extends MailDAO {
        ...
        DB.select("SELECT * FROM mail WHERE mail_recipient_id = ? ORDER BY recieved_time LIMIT 100", new ParamReadStH() {
            ...
            public void handleRead(ResultSet rset) throws SQLException {
                ...
                while (rset.next()) {
                        ...
                        for (Item item : mailboxItems) {
                            if (item.getObjectId() == attachedItemId) {
                                if (item.getItemTemplate().isArmor() || item.getItemTemplate().isWeapon()) {
                                    GDB.get(ItemStoneListDAO.class).load(Collections.singletonList(item));
                                }
                    ...
                    Letter letter = new Letter(mailUniqueId, recipientId, attachedItem, attachedKinahCount, mailTitle, mailMessage, senderName,
                        recievedTime, unread == 1, letterType);
    ...
    public boolean haveUnread(int playerId) {
        ...
        try {
            ...
            log.error("Could not read mail for player: " + playerId + " from GDB: " + e.getMessage(), e);
        } finally {
        ...
        DB.select("SELECT * FROM inventory WHERE `item_owner` = ? AND `item_location` = 127", new ParamReadStH() {
            ...
            public void handleRead(ResultSet rset) throws SQLException {
                while (rset.next()) {
                    ...
                    Item item = new Item(itemUniqueId, itemId, itemCount, itemColor, itemCreator, expireTime, activationCount, isEquiped == 1,
    ...
    private boolean saveLetter(final Timestamp time, final Letter letter) {
        ...
        return DB
            .insertUpdate(
                "INSERT INTO `mail` (`mail_unique_id`, `mail_recipient_id`, `sender_name`, `mail_title`, `mail_message`, `unread`, `attached_item_id`, `attached_kinah_count`, `express`, " +
                    "`recieved_time`) VALUES(?,?,?,?,?,?,?,?,?,?)",
                new IUStH() {
