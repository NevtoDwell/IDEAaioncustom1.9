...
public class MySQL5LegionDAO extends LegionDAO {
    ...
    private static final String UPDATE_LEGION_QUERY = "UPDATE legions SET name=?, level=?, contribution_points=?, deputy_permission=?, centurion_permission=?, legionary_permission=?, " +
        "volunteer_permission=?, disband_time=? WHERE id=?";
    /**
     ...
     * Storage Queries *
     */
    private static final String SELECT_STORAGE_QUERY = "SELECT `item_unique_id`, `item_id`, `item_count`, `item_color`, `item_creator`, `expire_time`, `activation_count`, `is_equiped`, `slot`, " +
        "`enchant`, `item_skin`, `fusioned_item`, `optional_socket`, `optional_fusion_socket`, `charge` FROM `inventory` WHERE `item_owner`=? AND `item_location`=? AND `is_equiped`=?";
    /**
     * History Queries *
     */
        ...
        DB.select(SELECT_STORAGE_QUERY, new ParamReadStH() {
            ...
            public void handleRead(ResultSet rset) throws SQLException {
                while (rset.next()) {
                    ...
                    Item item = new Item(itemUniqueId, itemId, itemCount, itemColor, itemCreator, expireTime, activationCount, isEquiped == 1, false,
