...
public class MySQL5inGameShopDAO extends InGameShopDAO {
    ...
    public static final String SELECT_QUERY = "SELECT `object_id`, `item_id`, `item_count`, `item_price`, `category`, `sub_category`, `list`, `sales_ranking`, `item_type`, `gift`, " +
        "`title_description`, `description` FROM `ingameshop`";
    ...
    public FastMap<Byte, List<IGItem>> loadInGameShopItems() {
        ...
        try {
            ...
            log.error("Could not restore inGameShop data for all from GDB: " + e.getMessage(), e);
        } finally {
    ...
    @Override
    public void saveIngameShopItem(int objectId, int itemId, long itemCount, long itemPrice, byte category,
                                   byte subCategory, int list, int salesRanking, byte itemType, byte gift, String titleDescription,
                                   String description) {
        Connection con = null;
        try {
            con = DatabaseFactory.getConnection();
