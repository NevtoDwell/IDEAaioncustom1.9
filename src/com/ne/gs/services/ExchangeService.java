...
import com.ne.gs.model.trade.Exchange;
import com.ne.gs.model.trade.ExchangeItem;
import com.ne.gs.network.aion.serverpackets.SM_DELETE_ITEM;
import com.ne.gs.network.aion.serverpackets.SM_EXCHANGE_ADD_ITEM;
import com.ne.gs.network.aion.serverpackets.SM_EXCHANGE_ADD_KINAH;
import com.ne.gs.network.aion.serverpackets.SM_EXCHANGE_CONFIRMATION;
import com.ne.gs.network.aion.serverpackets.SM_EXCHANGE_REQUEST;
...
public class ExchangeService {
    ...
    public void registerExchange(Player player1, Player player2) {
        ...
        player2.sendPck(new SM_EXCHANGE_REQUEST(player1.getName()));
        player1.sendPck(new SM_EXCHANGE_REQUEST(player2.getName()));
    }
    ...
    public void addKinah(Player activePlayer, long itemCount) {
        ...
        if (countToAdd > 0) {
            ...
            activePlayer.sendPck(new SM_EXCHANGE_ADD_KINAH(countToAdd, 0));
            partner.sendPck(new SM_EXCHANGE_ADD_KINAH(countToAdd, 1));
    ...
    public void addItem(Player activePlayer, int itemObjId, long itemCount) {
        ...
        if (item.getItemTemplate().isExchangeLimited() && item.getExchangeTime().isExpired()) {
            activePlayer.sendMsg("Item exchange time have been expired");
            return;
        }

        //        if (!item.getExchangeTime().isPermitted(partner))  {
        //            activePlayer.sendMsg("You are not allowed to exchange this item to " + partner.getName());
        //            return;
        //        }

        if (!item.isTradeable(activePlayer)) {
            if (item.getItemTemplate().isExchangeLimited()) {
                if (item.getExchangeTime().isExpired()) {
                    return;
                }
            } else {
                return;
        ...
        activePlayer.sendPck(new SM_EXCHANGE_ADD_ITEM(0, exchangeItem.getItem(), activePlayer));
        partner.sendPck(new SM_EXCHANGE_ADD_ITEM(1, exchangeItem.getItem(), partner));
    ...
    public void lockExchange(Player activePlayer) {
        ...
        if (exchange != null) {
            ...
            currentParter.sendPck(new SM_EXCHANGE_CONFIRMATION(3));
        }
    }
    ...
    public void cancelExchange(Player activePlayer) {
        ...
        if (currentParter != null) {
            currentParter.sendPck(new SM_EXCHANGE_CONFIRMATION(1));
        }
    }
    ...
    public void confirmExchange(Player activePlayer) {
        ...
        currentPartner.sendPck(new SM_EXCHANGE_CONFIRMATION(2));
    ...
    private void performTrade(Player activePlayer, Player currentPartner) {
        ...
        activePlayer.sendPck(new SM_EXCHANGE_CONFIRMATION(0));
        currentPartner.sendPck(new SM_EXCHANGE_CONFIRMATION(0));
    ...
    private boolean removeItemsFromInventory(Player player, Exchange exchange) {
        ...
        for (ExchangeItem exchangeItem : exchange.getItems().values()) {
            ...
            if (itemCount < itemInInventory.getItemCount()) {
                ...
                player.sendPck(new SM_DELETE_ITEM(itemInInventory.getObjectId()));
            }
        }
    ...
    public static final class ExchangeOpSaveTask implements Runnable {
        ...
        @Override
        public void run() {
            GDB.get(InventoryDAO.class).store(player1Items, player1Id);
            GDB.get(InventoryDAO.class).store(player2Items, player2Id);
        }
    }
