...
import com.ne.gs.model.team.legion.*;
import com.ne.gs.network.aion.serverpackets.*;
import com.ne.gs.utils.PacketSendUtility;
import com.ne.gs.utils.idfactory.IDFactory;
import com.ne.gs.world.World;
import com.ne.gs.world.container.LegionContainer;
import com.ne.gs.world.container.LegionMemberContainer;
...
public class LegionService {
    ...
    private void storeLegion(Legion legion, boolean newLegion) {
        if (newLegion) {
            ...
            GDB.get(LegionDAO.class).saveNewLegion(legion);
        } else {
            GDB.get(LegionDAO.class).storeLegion(legion);
            GDB.get(LegionDAO.class).storeLegionEmblem(legion.getLegionId(), legion.getLegionEmblem());
        }
    }
    ...
    private void storeLegionMember(LegionMember legionMember, boolean newMember) {
        if (newMember) {
            ...
            GDB.get(LegionMemberDAO.class).saveNewLegionMember(legionMember);
        } else {
            GDB.get(LegionMemberDAO.class).storeLegionMember(legionMember.getObjectId(), legionMember);
        }
    }
    ...
    private void deleteLegionFromDB(Legion legion) {
        ...
        GDB.get(LegionDAO.class).deleteLegion(legion.getLegionId());
    }
    ...
    private void deleteLegionMemberFromDB(LegionMemberEx legionMember) {
        ...
        GDB.get(LegionMemberDAO.class).deleteLegionMember(legionMember.getObjectId());
    ...
    public Legion getLegion(String legionName) {
         ...
         */
        Legion legion = GDB.get(LegionDAO.class).loadLegion(legionName);
    ...
    public Legion getLegion(int legionId) {
         ...
         * Else load the legion information from the database
         */
        Legion legion = GDB.get(LegionDAO.class).loadLegion(legionId);
    ...
    private void loadLegionInfo(Legion legion) {
         ...
         */
        legion.setLegionMembers(GDB.get(LegionMemberDAO.class).loadLegionMembers(legion.getLegionId()));

        /**
         * Load and set the announcement list
         */
        legion.setAnnouncementList(GDB.get(LegionDAO.class).loadAnnouncementList(legion.getLegionId()));

        /**
         * Set legion emblem
         */
        legion.setLegionEmblem(GDB.get(LegionDAO.class).loadLegionEmblem(legion.getLegionId()));

        /**
         * Load Legion Warehouse
         */
        legion.setLegionWarehouse(GDB.get(LegionDAO.class).loadLegionStorage(legion));
         ...
         * Load Legion History
         */
        GDB.get(LegionDAO.class).loadLegionHistory(legion);
    }
    ...
    public LegionMember getLegionMember(int playerObjId) {
        ...
        if (allCachedLegionMembers.contains(playerObjId)) {
            ...
            legionMember = GDB.get(LegionMemberDAO.class).loadLegionMember(playerObjId);
    ...
    public void disbandLegion(Legion legion) {
        for (Integer memberObjId : legion.getLegionMembers()) {
            LegionMemberEx m = getLegionMemberEx(memberObjId);
            if (m != null) {
                allCachedLegionMembers.remove(m);
            }
        }
    ...
    private LegionMemberEx getLegionMemberEx(int playerObjId) {
        if (allCachedLegionMembers.containsEx(playerObjId)) {
            ...
            LegionMemberEx legionMember = GDB.get(LegionMemberDAO.class).loadLegionMemberEx(playerObjId);
            // FIXME temp fix
            if (legionMember == null) {
                log.error("Unable to find legion member: " + playerObjId);
                return null;
            }
    ...
    private LegionMemberEx getLegionMemberEx(String playerName) {
        if (allCachedLegionMembers.containsEx(playerName)) {
            ...
            LegionMemberEx legionMember = GDB.get(LegionMemberDAO.class).loadLegionMemberEx(playerName);
            addCachedLegionMemberEx(legionMember);
            return legionMember;
        }
    }
     ...
     * @param activePlayer
     */
    public void requestDisbandLegion(final Creature npc, Player activePlayer) {
        ...
        if (legionRestrictions.canDisbandLegion(activePlayer, legion)) {
            ...
            if (disbandResult) {
                activePlayer.sendPck(new SM_QUESTION_WINDOW(SM_QUESTION_WINDOW.STR_GUILD_DISPERSE_STAYMODE, 0, 0));
            }
        }
    }
    ...
    public void createLegion(Player activePlayer, String legionName) {
        if (legionRestrictions.canCreateLegion(activePlayer, legionName)) {
             ...
             */
            activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_CREATED(legion.getLegionName()));
        }
    }
    ...
    private void invitePlayerToLegion(final Player activePlayer, final Player targetPlayer) {
        if (legionRestrictions.canInvitePlayer(activePlayer, targetPlayer)) {
            ...
            RequestResponseHandler responseHandler = new RequestResponseHandler(activePlayer) {
                ...
                public void acceptRequest(Creature requester, Player responder) {
                    if (!targetPlayer.getCommonData().isOnline()) {
                        activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_NO_SUCH_USER(targetPlayer.getName()));
                    } else {
                        ...
                        if (legion.addLegionMember(playerObjId)) {
                            ...
                            activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_INVITE_CAN_NOT_ADD_MEMBER_ANY_MORE);
                ...
                @Override
                public void denyRequest(Creature requester, Player responder) {
                    activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_INVITE_HE_REJECTED_INVITATION(targetPlayer.getName()));
                }
            };
            ...
            if (!requested) {
                activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_INVITE_OTHER_IS_BUSY);
            } else {
                activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_INVITE_SENT_INVITE_MSG_TO_HIM(targetPlayer.getName()));

                // Send question packet to buddy
                targetPlayer.sendPck(new SM_QUESTION_WINDOW(SM_QUESTION_WINDOW.STR_GUILD_INVITE_DO_YOU_ACCEPT_INVITATION, 0, 0, legion.getLegionName(), legion
    ...
    private void displayLegionMessage(Player targetPlayer, Entry<Timestamp, String> currentAnnouncement) {
        if (currentAnnouncement != null) {
            targetPlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_NOTICE(currentAnnouncement.getValue(), (int) (currentAnnouncement.getKey().getTime() / 1000)));
        }
    }
    ...
    private void appointBrigadeGeneral(final Player activePlayer, final Player targetPlayer) {
        if (legionRestrictions.canAppointBrigadeGeneral(activePlayer, targetPlayer)) {
            ...
            RequestResponseHandler responseHandler = new RequestResponseHandler(activePlayer) {
                ...
                public void acceptRequest(Creature requester, Player responder) {
                    if (!targetPlayer.getCommonData().isOnline()) {
                        activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_CHANGE_MASTER_NO_SUCH_USER);
                    } else {
                ...
                @Override
                public void denyRequest(Creature requester, Player responder) {
                    activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_CHANGE_MASTER_HE_DECLINE_YOUR_OFFER(targetPlayer.getName()));
                }
            };
            ...
            // If the player is busy and could not be asked
            if (!requested) {
                activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_CHANGE_MASTER_SENT_CANT_OFFER_WHEN_HE_IS_QUESTION_ASKED);
            } else {
                activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_CHANGE_MASTER_SENT_OFFER_MSG_TO_HIM(targetPlayer.getName()));

                // Send question packet to buddy
                // TODO: Add char name parameter? Doesn't work?
                targetPlayer.sendPck(new SM_QUESTION_WINDOW(SM_QUESTION_WINDOW.STR_GUILD_CHANGE_MASTER_DO_YOU_ACCEPT_OFFER, activePlayer.getObjectId(), 0, activePlayer
                    .getName()));
            }
        }
    }
    ...
    private void changeSelfIntro(Player activePlayer, String newSelfIntro) {
        if (legionRestrictions.canChangeSelfIntro(activePlayer, newSelfIntro)) {
            ...
            activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_WRITE_INTRO_DONE);
        }
    }
    ...
    private void updateAfterDisbandLegion(Legion legion) {
        for (Player onlineLegionMember : legion.getOnlineLegionMembers()) {
            ...
            onlineLegionMember.sendPck(new SM_LEGION_LEAVE_MEMBER(1300302, 0, legion.getLegionName()));
    ...
    private void updateMembersOfDisbandLegion(Legion legion, int unixTime) {
        for (Player onlineLegionMember : legion.getOnlineLegionMembers()) {
            onlineLegionMember.sendPck(new SM_LEGION_UPDATE_MEMBER(onlineLegionMember, 1300303, unixTime + ""));
    ...
    private void updateMembersOfRecreateLegion(Legion legion) {
        for (Player onlineLegionMember : legion.getOnlineLegionMembers()) {
            onlineLegionMember.sendPck(new SM_LEGION_UPDATE_MEMBER(onlineLegionMember, 1300307, ""));
    ...
    public void storeLegionEmblem(Player activePlayer, LegionEmblem customEmblem) {
        ...
        activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_CHANGE_EMBLEM);
    }
    ...
    public void storeLegionEmblem(Player activePlayer, int legionId, int emblemId, int color_r, int color_g, int color_b,
        ...
        if (legionRestrictions.canStoreLegionEmblem(activePlayer, legionId, emblemId)) {
            ...
            activePlayer.sendPck(new SM_LEGION_UPDATE_EMBLEM(legionId, emblemId, color_r, color_g, color_b, emblemType));
            activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_CHANGE_EMBLEM);
        }
    }
    ...
    /**
     */
    public void openLegionWarehouse(final Player player, Npc npc) {
        if (legionRestrictions.canOpenWarehouse(player)) {
            ...
            if (!isEmpty) {
                Partitioner.of(items, 10).foreach(new Partitioner.Func2<Item>() {
                    @Override
                    public boolean apply(List<Item> list, boolean first, boolean last) {
                        player.sendPck(new SM_WAREHOUSE_INFO(list, storageId, whLvl, first, player));
                        return true;
                    }
                });
            }
            player.sendPck(new SM_WAREHOUSE_INFO(null, storageId, whLvl, isEmpty, player));
            player.sendPck(new SM_DIALOG_WINDOW(npc.getObjectId(), 25));
        }
    }
     ...
     * @param npc
     */
    public void recreateLegion(final Npc npc, Player activePlayer) {
        final Legion legion = activePlayer.getLegion();
        if (legionRestrictions.canRecreateLegion(activePlayer, legion)) {
            ...
            if (disbandResult) {
                activePlayer.sendPck(new SM_QUESTION_WINDOW(SM_QUESTION_WINDOW.STR_GUILD_DISPERSE_STAYMODE_CANCEL, 0, 0));
            }
        }
    }
    ...
    public void LegionWhUpdate(Player player) {
        ...
        try {
             ...
             * 1. save items first
             */
            GDB.get(InventoryDAO.class).store(allItems, player.getObjectId(), player.getPlayerAccount().getId(), legion.getLegionId());
        ...
        if (legionRestrictions.canUploadEmblem(activePlayer)) {
            ...
            if (legionEmblem.getUploadSize() == legionEmblem.getUploadedSize()) {
                if (legionEmblem.getUploadedSize() == 0 || legionEmblem.getUploadSize() == 0) {
                    activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_WARN_CORRUPT_EMBLEM_FILE);
                    return;
                }
                if (!activePlayer.getInventory().tryDecreaseKinah(1130000L)) {
                    activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_MSG_NOT_ENOUGH_MONEY);
                    return;
                }

                // Finished
                legionEmblem.setCustomEmblemData(legionEmblem.getUploadData());
                GDB.get(LegionDAO.class).storeLegionEmblem(activePlayer.getLegion().getLegionId(), legionEmblem);
                LegionEmblem emblem = GDB.get(LegionDAO.class).loadLegionEmblem(activePlayer.getLegion().getLegionId());
    ...
    public void sendEmblemData(Player player, LegionEmblem legionEmblem, int legionId, String legionName) {
        player.sendPck(new SM_LEGION_SEND_EMBLEM(legionId, legionEmblem.getEmblemId(), legionEmblem.getColor_r(), legionEmblem.getColor_g(),
        ...
        do {
            ...
            if (currentSize >= maxSize) {
                ...
                player.sendPck(new SM_LEGION_SEND_EMBLEM_DATA(maxSize, bytes));
            } else {
                ...
                log.debug("legionEmblem data send size: " + (bytes.length) + " bytes");
                player.sendPck(new SM_LEGION_SEND_EMBLEM_DATA(currentSize, bytes));
            }
    ...
    /**
     * This will add a new announcement to the GDB and change the current announcement
     */
    private void changeAnnouncement(Player activePlayer, String announcement) {
        if (legionRestrictions.canChangeAnnouncement(activePlayer.getLegionMember(), announcement)) {
            ...
            activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_WRITE_NOTICE_DONE);
    ...
    private boolean storeNewAnnouncement(int legionId, Timestamp currentTime, String message) {
        return GDB.get(LegionDAO.class).saveNewAnnouncement(legionId, currentTime, message);
    }
    ...
    private void removeAnnouncement(int legionId, Timestamp key) {
        GDB.get(LegionDAO.class).removeAnnouncement(legionId, key);
    }
    ...
    public void addHistory(Legion legion, String text, LegionHistoryType legionHistoryType, int tabId, String description) {
        ...
        GDB.get(LegionDAO.class).saveNewLegionHistory(legion.getLegionId(), legionHistory);
    ...
    private void addLegionMember(Legion legion, Player player, LegionRank rank) {
        ...
        player.sendPck(new SM_LEGION_INFO(legion));

        // Send legion member info to the members
        SM_LEGION_MEMBERLIST.sendTo(player, loadLegionMemberExList(legion, player.getObjectId()));
        ...
        player.sendPck(new SM_LEGION_ADD_MEMBER(player, false, 0, ""));
            ...
            case 0x01:
                if (targetPlayer != null) {
                    if (targetPlayer.getPlayerSettings().isInDeniedStatus(DeniedStatus.GUILD)) {
                        activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_MSG_REJECTED_INVITE_GUILD(charName));
                        return;
                    }
                    invitePlayerToLegion(activePlayer, targetPlayer);
                } else {
                    activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_INVITE_NO_USER_TO_INVITE);
                }
                break;
            ...
            case LEGION_ACTION_KICK:
                ...
                if (legionRestrictions.canKickPlayer(activePlayer, charName)) {
                    if (removeLegionMember(charName, true, activePlayer.getName())) {
                        ...
                        if (targetPlayer != null) {
                            targetPlayer.sendPck(new SM_LEGION_LEAVE_MEMBER(1300246, 0, legion.getLegionName()));
                            targetPlayer.resetLegionMember();
                        }
                    }
                }
            ...
            case 0x02:
                if (legionRestrictions.canLeave(activePlayer)) {
                    if (removeLegionMember(activePlayer.getName(), false, "")) {
                        Legion legion = activePlayer.getLegion();
                        activePlayer.sendPck(new SM_LEGION_LEAVE_MEMBER(1300241, 0, legion.getLegionName()));
    ...
    public boolean removePlayerFromLegionAsItself(Player player) {
        if (removeLegionMember(player.getName(), false, "")) {
            Legion legion = player.getLegion();
            player.sendPck(new SM_LEGION_LEAVE_MEMBER(1300241, 0, legion.getLegionName()));
            player.resetLegionMember();
            return true;
        } else {
            return false;
    ...
    public void onLogin(Player activePlayer) {
        ...
        activePlayer.sendPck(new SM_LEGION_INFO(legion));

        // Send member list to player
        SM_LEGION_MEMBERLIST.sendTo(activePlayer, loadLegionMemberExList(legion, null));

        // Send current announcement to player
        displayLegionMessage(activePlayer, legion.getCurrentAnnouncement());

        if (legion.isDisbanding()) {
            activePlayer.sendPck(new SM_LEGION_EDIT(0x06, legion.getDisbandTime()));
        }
    }
    ...
    private class LegionRestrictions {
        ...
        private boolean canCreateLegion(Player activePlayer, String legionName) {
            ...
            if (!isValidName(legionName)) {
                activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_CREATE_INVALID_GUILD_NAME);
                return false;
            } // STR_GUILD_CREATE_TOO_FAR_FROM_CREATOR_NPC TODO
            else if (!isFreeName(legionName)) {
                activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_CREATE_SAME_GUILD_EXIST);
                return false;
            } else if (activePlayer.isLegionMember()) {
                activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_CREATE_ALREADY_BELONGS_TO_GUILD);
                return false;
            } else if (activePlayer.getInventory().getKinah() < LegionConfig.LEGION_CREATE_REQUIRED_KINAH) {
                activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_CREATE_NOT_ENOUGH_MONEY);
                return false;
            }
            return true;
        }
        ...
        private boolean canInvitePlayer(Player activePlayer, Player targetPlayer) {
            ...
            if (activePlayer.getLifeStats().isAlreadyDead()) {
                activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_INVITE_CANT_INVITE_WHEN_DEAD);
                return false;
            }
            if (isSelf(activePlayer, targetPlayer.getObjectId())) {
                activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_INVITE_CAN_NOT_INVITE_SELF);
                return false;
            } else if (targetPlayer.isLegionMember()) {
                if (legion.isMember(targetPlayer.getObjectId())) {
                    activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_INVITE_HE_IS_MY_GUILD_MEMBER(targetPlayer.getName()));
                } else {
                    activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_INVITE_HE_IS_OTHER_GUILD_MEMBER(targetPlayer.getName()));
                }
                return false;
        ...
        private boolean canKickPlayer(Player activePlayer, String charName) {
            ...
            if (isSelf(activePlayer, legionMember.getObjectId())) {
                activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_BANISH_CANT_BANISH_SELF);
                return false;
            } else if (legionMember.isBrigadeGeneral()) {
                activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_BANISH_CAN_BANISH_MASTER);
                return false;
            } else if (legionMember.getRank() == activePlayer.getLegionMember().getRank()) {
                activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_BANISH_DONT_HAVE_RIGHT_TO_BANISH);
                return false;
        ...
        private boolean canAppointBrigadeGeneral(Player activePlayer, Player targetPlayer) {
            ...
            if (!isBrigadeGeneral(activePlayer)) {
                activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_CHANGE_MEMBER_RANK_DONT_HAVE_RIGHT);
                return false;
            }
            if (isSelf(activePlayer, targetPlayer.getObjectId())) {
                activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_CHANGE_MASTER_ERROR_SELF);
                return false;
        ...
        private boolean canChangeLevel(Player activePlayer, long kinahAmount) {
            ...
            if (legion.getLegionLevel() == MAX_LEGION_LEVEL) {
                activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_CHANGE_LEVEL_CANT_LEVEL_UP);
                return false;
            } else if (activePlayer.getInventory().getKinah() < legion.getKinahPrice()) {
                activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_CHANGE_LEVEL_NOT_ENOUGH_MONEY);
                return false;
            } else if (!legion.hasRequiredMembers()) {
                activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_CHANGE_LEVEL_NOT_ENOUGH_MEMBER);
                return false;
            } else if (legion.getContributionPoints() < levelContributionPrice) {
                activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_CHANGE_LEVEL_NOT_ENOUGH_POINT);
                return false;
            }
            return true;
        }
        ...
        private boolean canChangeAnnouncement(LegionMember legionMember, String announcement) {
            return !(!isValidAnnouncement(announcement) && legionMember.hasRights(LegionPermissionsMask.EDIT));
        }
        ...
        private boolean canDisbandLegion(Player player, Legion legion) {
            ...
            if (!isBrigadeGeneral(player)) {
                player.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_DISPERSE_ONLY_MASTER_CAN_DISPERSE);
                return false;
            } else if (legion.getLegionWarehouse().size() > 0) {
                // TODO: Can't disband during using legion warehouse!!
                return false;
            } else if (legion.isDisbanding()) {
                player.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_DISPERSE_ALREADY_REQUESTED);
                return false;
            } else if (legion.getLegionWarehouse().size() > 0) {
                player.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_DISPERSE_CANT_DISPERSE_GUILD_STORE_ITEM_IN_WAREHOUSE);
                return false;
            }
            return true;
        }
        ...
        private boolean canLeave(Player activePlayer) {
            if (isBrigadeGeneral(activePlayer)) {
                activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_LEAVE_MASTER_CANT_LEAVE_BEFORE_CHANGE_MASTER);
                return false;
            }
            return true;
        }
        ...
        private boolean canRecreateLegion(Player activePlayer, Legion legion) {
            if (!isBrigadeGeneral(activePlayer)) {
                activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_DISPERSE_ONLY_MASTER_CAN_DISPERSE);
                return false;
            } else if (!legion.isDisbanding()) {
        ...
        public boolean canOpenWarehouse(Player activePlayer) {
            ...
            if (legion.isDisbanding()) {
                activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_GUILD_WAREHOUSE_CANT_USE_WHILE_DISPERSE);
                return false;
            ...
            if (whUser != playerId && legWh.getWhUser() != 0) {
                activePlayer.sendPck(new SM_SYSTEM_MESSAGE(1300280));
                return false;
            }
        ...
        public boolean canStoreLegionEmblem(Player activePlayer, int legionId, int emblemId) {
            ...
            if (emblemId < MIN_EMBLEM_ID || emblemId > MAX_EMBLEM_ID) {
                ...
                activePlayer.sendPck(SM_SYSTEM_MESSAGE.STR_MSG_NOT_ENOUGH_KINA(LegionConfig.LEGION_EMBLEM_REQUIRED_KINAH));
                return false;
            }
            return true;
        }
        ...
        private boolean isFreeName(String name) {
            return !GDB.get(LegionDAO.class).isNameUsed(name);
        }
