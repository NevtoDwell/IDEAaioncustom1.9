...
import com.ne.gs.model.broker.BrokerItemMask;
import com.ne.gs.model.broker.BrokerMessages;
import com.ne.gs.model.broker.BrokerPlayerCache;
...
import com.ne.gs.network.aion.serverpackets.SM_BROKER_SERVICE;
...
public class BrokerService {
    ...
    private void initBrokerService() {
        ...
        List<BrokerItem> brokerItems = GDB.get(BrokerDAO.class).loadBroker();
    ...
    public void showRequestedItems(Player player, int clientMask, int sortType, int startPage, List<Integer> itemList) {
        ...
        player.sendPck(new SM_BROKER_SERVICE(searchItems, totalSearchItemsCount, startPage));
    }
    ...
    public void buyBrokerItem(Player player, int itemUniqueId) {
        if(player.getController().isInShutdownProgress()){
            player.sendMsg("Во время рестарта нельзя покупать!");
            return;
        }
        ...
        if (SecurityConfig.BROKER_PREBUY_CHECK) {
            if (!GDB.get(BrokerDAO.class).preBuyCheck(itemUniqueId)) {
                player.sendMsg("Sorry, but this item already sold");
                return;
            }
        }

        if (buyingItem.getSellerId() == player.getObjectId()) {
            player.sendPck(SM_SYSTEM_MESSAGE.STR_VENDOR_CAN_NOT_BUY_MY_REGISTER_ITEM);
            return;
        }
    ...
    private void putToSettled(Race race, BrokerItem brokerItem, boolean isSold) {
        ...
        if (seller != null) {
            seller.sendPck(new SM_BROKER_SERVICE(true, getTotalSettledKinah(seller)));
    ...
    public void registerItem(Player player, int itemUniqueId, long price, int itemCount) {
        ...
        if(player.getController().isInShutdownProgress()){
            player.sendMsg("Во время рестарта нельзя регистрировать предметы!");
            return;
        }
        if (itemToRegister == null || itemToRegister.getItemCount() < itemCount ) {
            return;
        }
        ...
        if (registeredItemsCount > 14) {
            player.sendPck(new SM_BROKER_SERVICE(BrokerMessages.NO_SPACE_AVAIABLE.getId()));
            return;
        ...
        if (player.getInventory().getKinah() < registrationCommition) {
            player.sendPck(new SM_BROKER_SERVICE(BrokerMessages.NO_ENOUGHT_KINAH.getId()));
            return;
        }

        player.getInventory().decreaseKinah(registrationCommition);
        
		if (itemToRegister.getItemTemplate().isStackable() && itemCount < itemToRegister.getItemCount()) {
			player.getInventory().decreaseItemCount(itemToRegister, itemCount);
			itemToRegister = ItemFactory.newItem(itemToRegister.getItemId(), itemCount);
		}
		else {
			player.getInventory().remove(itemToRegister);
			PacketSendUtility.sendPck(player, new SM_DELETE_ITEM(itemToRegister.getObjectId()));
		}
        ...
        player.sendPck(new SM_BROKER_SERVICE(newBrokerItem, 0));
    }
    ...
    public void showRegisteredItems(Player player) {
        ...
        AionServerPacket packet = new SM_BROKER_SERVICE(registeredItems.toArray(new BrokerItem[registeredItems.size()]));
    ...
    public void cancelRegisteredItem(Player player, int brokerItemId) {
        ...
        if(player.getController().isInShutdownProgress()){
            player.sendMsg("Нельзя забирать предметы во время рестарта!");
            return;
        }
        if (brokerItem != null) {
     ...
     * @param player
     */
    public void showSettledItems(final Player player) {
        ...
        // [client limitation] accepts 99 items at max
        final long finalTotalKinah = totalKinah;
        Partitioner.of(settledItems, 99).foreach(new Partitioner.Func<BrokerItem>() {
            @Override
            public boolean apply(List<BrokerItem> list) {
                player.sendPck(new SM_BROKER_SERVICE(list.toArray(new BrokerItem[list.size()]), finalTotalKinah));
                return false;
            }
        });
    }
    ...
    public void settleAccount(Player player) {
        ...
        if (!itemsLeft) {
            player.sendPck(new SM_BROKER_SERVICE(false, 0));
        }
    ...
    public void onPlayerLogin(Player player) {
        ...
        for (BrokerItem item : brokerSettledItems.values()) {
            if (item != null && playerId == item.getSellerId()) {
                player.sendPck(new SM_BROKER_SERVICE(true, getTotalSettledKinah(player)));
                break;
            }
        }
    }
    ...
    public static final class BrokerOpSaveTask implements Runnable {
        ...
        public void run() {
            ...
            if (item != null) {
                GDB.get(InventoryDAO.class).store(item, playerId);
            }
            if (brokerItem != null) {
                GDB.get(BrokerDAO.class).store(brokerItem);
            }
            if (kinahItem != null) {
                GDB.get(InventoryDAO.class).store(kinahItem, playerId);
            }
        }
    }
