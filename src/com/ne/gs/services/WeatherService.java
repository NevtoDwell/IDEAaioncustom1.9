...
import com.ne.gs.model.templates.world.WeatherTable;
...
import com.ne.gs.network.aion.serverpackets.SM_WEATHER;
...
/**
 * This service in future should schedule job that is changing weather sometimes in region and probably sends to all players
 *
 * @author ATracer
 * @author Kwazar
 */
        ...
        if (player == null) {
            for (Iterator<Player> playerIterator = World.getInstance().getPlayersIterator(); playerIterator.hasNext();) {
                ...
                if (currentPlayer.getWorldId() == mapId) {
                    PacketSendUtility.sendPck(currentPlayer, new SM_WEATHER(weatherEntries));
                }
            }
        }
        else {
            PacketSendUtility.sendPck(player, new SM_WEATHER(weatherEntries));
        }
    }
