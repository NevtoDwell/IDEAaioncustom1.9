...
public class EffectController implements Iterable<Effect>{
	...
	if (remove) {
	  if (removePower(effect, power)) {
	    ...
	    abnormalEffectMap.remove(effect.getStack());
	    count--;
	  } else if (effect.getEffector() instanceof Player) {
	    PacketSendUtility.sendPck((Player) effect.getEffector(), SM_SYSTEM_MESSAGE.STR_MSG_NOT_ENOUGH_DISPELCOUNT);
	  }

	} else if (effect.getEffector() instanceof Player) {
	  PacketSendUtility.sendPck((Player) effect.getEffector(), SM_SYSTEM_MESSAGE.STR_MSG_NOT_ENOUGH_DISPELLEVEL);
	}
