...
import com.ne.gs.model.templates.itemgroups.*;
import com.ne.gs.model.templates.pet.FoodType;
import com.ne.gs.model.templates.rewards.CraftItem;
import com.ne.gs.model.templates.rewards.CraftRecipe;
...
@XmlType(name = "", propOrder = {"craftMaterials", "craftShop", "craftBundles", "craftRecipes", "manastonesCommon", "manastonesRare", "medals", "foodCommon",
                                 "foodRare", "foodLegendary", "medicineCommon", "medicineRare", "medicineLegendary", "oresRare", "oresLegendary", "oresUnique", "oresEpic", "gatherRare",
                                 "enchants", "bossRare", "bossLegendary", "feedFluids", "feedArmor", "feedThorns", "feedBones", "feedBalaurScales", "feedSouls", "feedExcludes",
                                 "stinkingJunk", "healthyFoodAll", "healthyFoodSpicy", "aetherPowderBiscuit", "aetherCrystalBiscuit", "aetherGemBiscuit", "poppySnack", "poppySnackTasty",
...
public class ItemGroupsData {
    ...
    TIntObjectHashMap<Map<Range<Integer>, List<CraftReward>>> craftMaterialsBySkill = new TIntObjectHashMap<>();
    TIntObjectHashMap<Map<Range<Integer>, List<CraftReward>>> craftShopBySkill = new TIntObjectHashMap<>();
    TIntObjectHashMap<Map<Range<Integer>, List<CraftReward>>> craftBundlesBySkill = new TIntObjectHashMap<>();
    TIntObjectHashMap<Map<Range<Integer>, List<CraftReward>>> craftRecipesBySkill = new TIntObjectHashMap<>();
    ...
    Map<FoodType, Set<Integer>> petFood = new THashMap<>();
    ...
    void afterUnmarshal(Unmarshaller u, Object parent) {
        ...
        for (FoodType foodType : FoodType.values()) {
            ...
            if (food != null) {
                Set<Integer> itemIds = new THashSet<>();
    ...
    void MapCraftReward(TIntObjectHashMap<Map<Range<Integer>, List<CraftReward>>> dataHolder, CraftReward reward) {
    ...
    public int bonusSize() {
        return count + manastonesCommon.getItems().size() + manastonesRare.getItems().size() + foodCommon.getItems()
            .size() + foodRare
            .getItems().size()
            + foodLegendary.getItems().size() + medicineCommon.getItems().size() + medicineRare.getItems()
            .size() + medicineLegendary
            .getItems().size()
            + oresRare.getItems().size() + oresLegendary.getItems().size() + oresUnique.getItems().size() + oresEpic
            .getItems().size()
            + gatherRare.getItems().size() + enchants.getItems().size() + bossRare.getItems().size() + bossLegendary
            .getItems().size();
    }
