...
import com.ne.gs.model.templates.npcshout.ShoutGroup;
import com.ne.gs.model.templates.npcshout.ShoutList;
...
public class NpcShoutData {
    ...
    @XmlTransient
    private final TIntObjectHashMap<TIntObjectHashMap<List<NpcShout>>> shoutsByWorldNpcs = new TIntObjectHashMap<>();
    ...
    public void afterUnmarshal(Unmarshaller u, Object parent) {
        for (ShoutGroup group : shoutGroups) {
            for (int i = group.getShoutNpcs().size() - 1; i >= 0; i--) {
                ...
                TIntObjectHashMap<List<NpcShout>> worldShouts = shoutsByWorldNpcs.get(worldId);
                if (worldShouts == null) {
                    worldShouts = new TIntObjectHashMap<>(1);
    ...
    public List<NpcShout> getNpcShouts(int worldId, int npcId) {
        TIntObjectHashMap<List<NpcShout>> worldShouts = shoutsByWorldNpcs.get(0);
